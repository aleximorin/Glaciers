...
def getparser():
    description = "Minimize mismatch between an inversion method output and GPR data using GlaTe Algorithm from Langhammer et al. (2019)"
...
def main():
    ...
    # if -plots flage is passed then plots are generated
    if plots:
        ...
        GOI.plot_map(
            im=GOI_inv_glate.thickness,  # The 3D array to be plotted
            title=f"${alpha_gpr:2.2f} \cdot$ ({GOI_inv.name})",  # The title of the plot
            cbar_unit="[m]",  # The color bar's units
            tag=GOI_inv_glate.tag,  # The plot's tag i.e. the name of the saved file
            outline=True,
            points=True,
            point_color=True,
        )  # PlottiGOI parameters
